...
type Row = { id: string; nama: string; email: string; role: UserRole; aktif: boolean; createdAt: string; lastSignInAt: string | null };
...
export function PenggunaTable({ currentUserId }: { currentUserId: string }) {
  ...
  const AktifToggle = ({ r }: { r: Row }) => (
    <button type="button" aria-pressed={r.aktif} aria-label={`${r.aktif ? 'Nonaktifkan' : 'Aktifkan'} ${r.nama}`}
      ...
      <Power size={16} weight="bold" /> {r.aktif ? 'Aktif' : 'Nonaktif'}
    </button>
  );
    ...
    <section className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <button type="button" onClick={() => setModal(true)} className="h-11 px-4 rounded-2xl bg-teal-600 hover:bg-teal-700 text-white font-bold inline-flex items-center gap-2">
          <UserPlus size={20} weight="bold" /> Undang pengguna
        </button>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-b border-slate-100 dark:border-slate-800/60 last:border-0">
                ...
                <td className="px-5 py-3 text-slate-500">{fmt(r.lastSignInAt)}</td>
              </tr>
            ))}
      ...
      <ul className="md:hidden space-y-3">
        {rows.map(r => (
          <li key={r.id} className="p-4 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3">
            ...
            <p className="text-[11px] text-slate-500">Terakhir masuk: {fmt(r.lastSignInAt)}</p>
          </li>
        ))}
      </ul>
