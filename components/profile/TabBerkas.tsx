...
function BarisBerkas({ label, dok, onBuka }: { label: string; dok?: Dok; onBuka: (d: Dok) => void }) {
  ...
  const isi = (
    <>
      ...
      {dok && <CaretRight size={16} weight="bold" className="shrink-0 text-bq-redup" aria-hidden="true" />}
    </>
  );
  return dok
...
export function TabBerkas({ santri }: { santri: { id: string; namaLengkap: string; documents?: Dok[] } }) {
  ...
  const buka = (label: string) => (d: Dok) => setPratinjau({ judul: `${label} - ${santri.namaLengkap}`, dok: d });
  ...
  return (
    <div className="space-y-4">
      ...
      <DocumentPreviewModal isOpen={!!pratinjau} onClose={() => setPratinjau(null)} title={pratinjau?.judul ?? ''}
